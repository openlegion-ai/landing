...
export function DashboardPreview() {
  ...
  const STATE_TRANSLATE: Record<string, string> = {
  ...
  const HEALTH_LABEL: Record<string, string> = {
    healthy: t("healthOnline"),
    unhealthy: t("healthDegraded"),
  };
        ...
        <div className="dashboard-tilt dashboard-frame rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-2 border-b border-border/50 bg-[#0d0e14] px-4 py-3">
            ...
            <div className="ml-3 flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-foreground/70">{t("windowTitle")}</span>
            </div>
          ...
          <div className="border-b border-border/30 bg-[#0b0c11] px-4 py-2.5 sm:px-5">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted">
              ...
              <span>
                <span className="font-mono font-medium text-foreground/80 tabular-nums">${totalCost.toFixed(2)}</span> {t("todaySuffix")}
              </span>
              <span className="text-border">&middot;</span>
              <span>
                <span className="font-mono font-medium text-foreground/80 tabular-nums">{totalTokens.toLocaleString()}</span> {t("tokensSuffix")}
              </span>
              <div className="hidden items-center gap-3 sm:flex sm:ml-auto">
                {healthyCount > 0 && (
                  <span className="flex items-center gap-1">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
                    {t("onlineCount", { count: healthyCount })}
                  </span>
                )}
                {unhealthyCount > 0 && (
                  <span className="flex items-center gap-1">
                    <span className="h-1.5 w-1.5 rounded-full bg-amber-400" />
                    {t("degradedCount", { count: unhealthyCount })}
                  </span>
                )}
              </div>
            </div>
