...
description: "AI agent access control: RBAC and ABAC models, least-privilege tool-level ACLs, vault credential scoping, inter-agent trust tokens, OWASP LLM06, NIST SP 800-207 Zero Trust, and EU AI Act Article 9."
...
### OWASP LLM06: Excessive Agency and Autonomous Blast Radius

**OWASP LLM Top 10 v2 (2025) LLM06: Excessive Agency** occurs when an AI agent is granted more permissions than it needs for its current task. Three contributing factors:
...
**PBAC** externalizes the permission decision to a policy engine, and the agent runtime calls the policy engine with request attributes and receives an allow/deny decision and structured reason string.
...
| **Human approval gate on destructive tools: irreversible actions queued for approval; OWASP LLM06 Excessive Autonomy mitigated** | Yes: approval queue | No | No | No | No |
