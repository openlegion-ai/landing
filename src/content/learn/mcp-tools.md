...
**Control 3: Name collision rejection.** If two tool hosts expose a tool with the same name, reject the connection that causes the collision. Ambiguous tool sets enable shadowing attacks, where an attacker registers a tool with the same name as a trusted tool to intercept invocations.
...
This blocks SSRF attacks: a malicious tool result that attempts to reach internal metadata endpoints (AWS IMDS at 169.254.169.254) is blocked at the network layer.
...
Anomalous tool invocation detected
    ...
    +- Is the anomaly in invocation volume?
        YES -> Apply rate limit (50 invocations/minute cap)
              Alert operator if volume exceeds 10x baseline
              Auto-terminate session if volume exceeds 20x baseline
```

**Credential rotation on tool handover:** when an agent session ends or an agent is replaced in a workflow, rotate the credentials associated with any tools that agent had access to. A credential scoped to a specific agent's session should not remain valid after that session closes.
