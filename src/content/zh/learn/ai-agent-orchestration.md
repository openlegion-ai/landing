...
description: "AI 智能体编排平台，提供舰队模型协调（黑板 + 发布/订阅 + 交接）、容器隔离、凭证密钥库和单智能体预算控制。"
...
- **舰队模型协调（黑板 + 发布/订阅 + 交接）** — OpenLegion 使用 YAML 定义的有向无环图进行任务路由。没有 LLM "CEO 智能体"做不透明的路由决策。
- **集群模型编排** — 通过舰队模型协调（黑板 + 发布/订阅 + 交接） 实现顺序和并行执行，配合黑板协调和发布/订阅消息传递。集群模型，而非层级结构。
...
这就是为什么 OpenLegion 将 [AI 智能体安全](/learn/ai-agent-security) 和编排视为同一个系统，而不是事后拼接的独立模块。
...
OpenLegion 使用集群模型，而非层级结构。所有智能体通过集中式黑板（SQLite 支持的共享状态）进行通信，发布/订阅消息由网格主机处理。没有"CEO 智能体"或主管智能体做路由决策——舰队模型协调 定义执行顺序，黑板提供智能体在执行过程中读写的共享上下文。这使协调保持确定性且可审计。

## 为什么隔离、密钥库和预算控制是编排的关注点

大多数 [AI 智能体框架](/learn/ai-agent-frameworks) 将安全视为编排工作完成后才添加的内容。智能体路由是一个模块。凭证管理是另一个关注点。成本追踪是可观测性的附加组件。
...
在舰队模型协调（黑板 + 发布/订阅 + 交接）中，每个智能体都有特定的权限集。每个智能体的 ACL 矩阵定义它可以调用哪些工具、访问哪些文件以及允许执行哪些网格操作。编排器在每个转换点执行这些约束。
...
**步骤 4：部署。**
`openlegion start` 配置三个隔离容器，通过密钥库代理为每个容器注入相应凭证，并启动舰队。仪表板显示每个智能体的实时 token 使用、成本追踪和流式输出。

**步骤 5：监控和审计。**
舰队模型协调意味着每个工作流步骤都是显式且可追踪的。内置的请求追踪系统记录任务转换、工具调用和 token 支出，提供实时可观测性——无需解析不透明的 LLM 决策日志。

## AI 智能体编排工具对比

| 能力 | OpenLegion | LangGraph | CrewAI | AutoGen |
|---|---|---|---|---|
| **编排模型** | 舰队模型协调（黑板 + 发布/订阅 + 交接） | 编程式 StateGraph | 基于角色的团队 + 事件驱动 Flows | 基于对话的群聊 |
| **智能体隔离** | 每个智能体一个 Docker 容器（强制） | 无内置 | 共享 Python 进程 | Docker 仅用于代码执行 |
| **凭证管理** | 密钥库代理——盲注 | 环境变量 | 环境变量 | 环境变量 |
| **预算控制** | 单智能体每日/每月硬性截止 | 无 | 无 | 无 |
| **任务路由** | 舰队模型——黑板 + 发布/订阅 + 交接（无 CEO 智能体） | 条件边（代码定义） | 层级管理智能体或顺序执行 | RoundRobin、Selector、Swarm、GraphFlow |
...
更深入的对比请参见我们完整的 [AI 智能体框架对比](/learn/ai-agent-frameworks)。
...
AI 智能体编排平台提供用于协调多智能体工作流的托管基础设施。除了基本路由外，平台还处理容器配置、凭证密钥库、成本追踪和可观测性。OpenLegion 是一个 [AI 智能体平台](/learn/ai-agent-platform)，将编排和治理视为同一个系统——每个路由决策都经过隔离和成本控制。

### 如何在生产环境中编排多个 AI 智能体？

在生产环境中，多智能体编排需要在可用原型之外具备四项能力：运行时隔离（每个智能体在自己的容器中）、凭证分离（智能体间不共享 API 密钥）、预算执行（单智能体成本限额和硬性截止）和确定性路由（可审计的任务流）。OpenLegion 通过部署在隔离 Docker 容器中的 YAML 定义的 舰队模型协调以及用于凭证管理的密钥库代理来处理所有四项。
...
LLM 驱动的编排使用 AI 模型（"CEO 智能体"）在运行时决定哪个智能体处理每个任务。这很灵活但不透明——你无法提前预测或审计路由决策。确定性编排使用预定义规则（OpenLegion 中的 舰队模型协调），在任何智能体运行之前即可审计。你确切知道哪个智能体处理什么、在什么条件下、拥有什么权限。
...
LangGraph 使用编程式 StateGraph，其中节点是 Python 函数，边定义转换。它对状态和流程提供强大的控制，但不提供内置隔离、凭证管理或成本控制。OpenLegion 使用舰队模型协调——黑板 + 发布/订阅 + 交接——并将容器隔离、密钥库代理凭证注入和单智能体预算作为原生编排特性。LangGraph 提供更多编程灵活性；OpenLegion 将治理作为一等编排关注点。
