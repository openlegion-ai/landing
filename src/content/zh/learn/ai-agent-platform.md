...
你需要请求追踪和工作流可观测性、能在智能体被攻陷时保持凭证隔离、以及防止成本失控的预算控制。OpenLegion 的架构专为需要 SOC 2 级别控制的环境设计。确定性 DAG 执行意味着每个工作流步骤都是显式且可追踪的——控制平面中没有不透明的 LLM 决策。详见我们的 [AI 智能体安全](/learn/ai-agent-security) 页面，了解完整的威胁模型。
