...
- **确定性编排** — YAML 定义的 舰队模型协调用于任务路由。没有"CEO 智能体"做不透明的决策。
...
**托管式 AI 智能体平台**处理运维层，让你的团队专注于智能体逻辑。OpenLegion 属于这一类——但有一个关键区别：它在 PolyForm Perimeter License 1.0.1 下开源可用。你获得平台级运维（隔离、密钥库、预算控制），同时在基础设施层面不被厂商锁定。
...
- 你需要 [AI 智能体安全](/learn/ai-agent-security) 保障，但不想自己构建
...
你需要请求追踪和工作流可观测性、能在智能体被攻陷时保持凭证隔离、以及防止成本失控的预算控制。OpenLegion 的架构专为需要 纵深防御的环境设计。舰队模型协调意味着每个工作流步骤都是显式且可追踪的——控制平面中没有不透明的 LLM 决策。详见我们的 [AI 智能体安全](/learn/ai-agent-security) 页面，了解完整的威胁模型。
...
| **编排** | 自己编写路由逻辑或使用 LLM 路由 | YAML 定义的 舰队模型协调——确定性、可审计 |
...
| **浏览器自动化** | 配置 Playwright/Puppeteer，管理 Chrome 实例 | 每智能体独立的 Camoufox（在共享浏览器服务容器中）（隐匿 Firefox）浏览器服务，支持 KasmVNC、CDP 控制、自动恢复 |
...
总结：如果你在评估 [AI 智能体框架](/learn/ai-agent-frameworks) 时发现自己构建的基础设施比智能体逻辑还多，那你就是在用框架工具解决平台问题。OpenLegion 处理平台层，让你可以专注于智能体真正要做的事情。
...
## 架构：四区信任模型加上一个操作员或内部权限层

OpenLegion 将每个部署分为四层信任区域加上一个操作员或内部权限层域：
...
这种架构意味着 [AI 智能体编排](/learn/ai-agent-orchestration) 和安全不是两个独立的关注点——它们是同一个系统。
...
托管式 AI 智能体平台为你处理容器配置、凭证密钥库、成本控制和可观测性。自托管意味着你在自己的基础设施上部署框架（LangGraph、CrewAI、AutoGen），并自己构建这些运维层。托管方式更快进入生产，需要更少的 DevOps 投入。自托管提供最大的基础设施控制权。OpenLegion 提供了一种混合方案：可自托管的源码可用代码（PolyForm Perimeter License 1.0.1），内置托管平台功能。

### OpenLegion 与其他 AI 智能体平台相比如何？

OpenLegion 以安全优先架构作为差异化优势。根据撰写时的公开文档，没有其他主要 [AI 智能体框架](/learn/ai-agent-frameworks) 提供内置的盲注凭证注入、强制性的单智能体容器隔离或原生的单智能体预算执行。请参阅我们的[框架对比](/learn/ai-agent-frameworks)，了解 OpenClaw、LangGraph、CrewAI、AutoGen 和 Semantic Kernel 的详细对比。

### OpenLegion 使用什么许可证？

OpenLegion 在 PolyForm Perimeter License 1.0.1下开源可用，发布在 [GitHub](https://github.com/openlegion-ai/openlegion) 上。该项目还为希望获得托管基础设施而无需自托管的团队提供托管平台。
