...
可以。OpenLegion 在 PolyForm Perimeter License 1.0.1下源码可用。自托管需要 Python 3.10+ 和 Docker。安装流程为 `git clone && ./install.sh && openlegion start`；首次 Docker 镜像构建需要几分钟。不需要外部服务——无 Redis、无 Kubernetes、无云服务。可在单机上运行。对于偏好托管基础设施的团队，也提供托管选项。
