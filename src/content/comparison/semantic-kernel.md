...
SK filters provide application-level governance (PII redaction, content blocking, logging). OpenLegion provides infrastructure-level security (container isolation, vault proxy, resource caps). These are complementary layers; SK filters govern what agents do while OpenLegion constrains what agents can access. See our [AI agent security](/learn/ai-agent-security) page for the full threat model.
